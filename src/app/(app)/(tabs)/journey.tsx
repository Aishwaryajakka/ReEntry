...
function ActivityRow({
  ...
  const toggle = () => {
    ...
    height.value = withTiming(expanded ? 0 : 1, { duration: reducedOrLowStim ? 0 : 180 });
  };

  const detailStyle = useAnimatedStyle(() => ({
    opacity: reducedOrLowStim ? (expanded ? 1 : 0) : height.value,
  ...
  return (
    ...
    >
      <View className="flex-row items-center gap-3">
        ...
        >
          <CategoryIcon category={log.activityCategory} size={20} color={theme.foreground} />
        </View>
        ...
        {expanded ? <ChevronDown size={16} color={theme.foregroundMuted} /> : <ChevronRight size={16} color={theme.foregroundMuted} />}
      </View>

      <Animated.View style={detailStyle}>
        <View className="pt-2 pl-13">
          {log.challengeTagIds.length > 0 && (
            <DataBadgeList tagIds={log.challengeTagIds} allTags={allTags} className="mb-2" />
          )}
          {log.notes ? (
            <LabelText className="italic leading-5 text-muted-foreground">"{log.notes}"</LabelText>
          ) : null}
          <Pressable
            onPress={onPress}
          ...
          >
            <MicroText className="text-foreground underline">Edit activity</MicroText>
          </Pressable>
        </View>
      </Animated.View>
    </Pressable>
...
export default function JourneyScreen() {
  const { activityLogs, challengeTags, dailyCheckIns, today, lowStimulationMode } = useAppContext();
  ...
  const sections = useMemo<Section[]>(() => {
    ...
    return Array.from(byDate.entries())
      .map(([date, logs]) => ({ date, data: logs.sort((a, b) => a.id.localeCompare(b.id)) }))
  ...
  return (
    <ScreenShell>
      ...
      <HeadingText className="mb-2 leading-tight text-foreground">Your activity{"\n"}timeline</HeadingText>
      ...
      {totalCount === 0 ? (
        <SectionCard className="mb-5 items-center py-8">
          <Text className="text-xl font-semibold text-foreground mb-2">No activities yet</Text>
          <MicroText className="text-center leading-5 mb-5 px-4">
            Your activity timeline will appear here once you log your first activity on the Today tab.
          </MicroText>
          <PrimaryButton
        ...
        <>
          <SectionCard className="mb-5 border-l-4 border-l-accent">
            ...
            <LabelText className="leading-5">
              You reported {totalCount} activit{totalCount !== 1 ? 'ies' : 'y'} across {uniqueDates} date{uniqueDates !== 1 ? 's' : ''}. Tap any activity to see details, or long-press to edit.
            </LabelText>
          </SectionCard>
            ...
            renderSectionHeader={({ section: { date } }) => (
              <View className="py-2 border-b border-border">
                <Text className="text-sm font-bold text-foreground">
                  {date === today ? 'Today' : formatWeekday(date)} · {formatShort(date)}
                </Text>
              </View>
            )}
            renderItem={({ item }) => (
            ...
            ListFooterComponent={
              <View className="mt-4 px-1">
                <MicroText className="text-center leading-5 text-muted-foreground">
                  These records reflect what you reported. ReEntry does not estimate recovery time, predict outcomes, or provide medical advice.
                </MicroText>
              </View>
            }
          />
      ...
      <ActivityLogModal visible={modalOpen} onClose={closeEdit} log={editingLog} />
    </ScreenShell>
  );
}
